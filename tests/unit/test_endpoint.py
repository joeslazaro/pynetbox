...
class EndPointTestCase(unittest.TestCase):
    ...
    def test_bulk_update_records(self):
        with patch(
            ...
            app = Mock(name="test")
            test_obj = Endpoint(api, app, "test")
            objects = [Record({'id': i, 'name': 'dummy' + str(i), 'unchanged': 'yes'}, api, test_obj) for i in ids]
            for o in objects:
                o.name = 'fluffy'+str(o.id)
            mock.return_value = [o.serialize() for o in objects]
            test = test_obj.update(objects)
            mock.assert_called_with(verb='patch', data=[{'id': i, 'name': 'fluffy' + str(i)} for i in ids])
            self.assertTrue(test)
